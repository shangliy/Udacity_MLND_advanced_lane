...
def process_image(image):
    ...
    grady = utils.abs_sobel_thresh(indist, orient='y', sobel_kernel=ksize, thresh=(20, 250))
    ...
    s_binary = utils.hls_select(indist, thresh=(80, 255))
    ...
    if (frame < frame_cut):
        ...
        for yl in range(1,distance+1):
            ...
            if np.sum(np.sum(warped[yvals-30:yvals,win_left-15:win_left+15], axis=0)) < 1:

                leftx[yvals] = win_left
                win_left = win_left
            else:
                lx = win_left-15 + np.argmax(np.sum(warped[yvals-30:yvals,win_left-15:win_left+15], axis=0))
                leftx[yvals] = lx
                win_left = lx


            if np.sum(np.sum(warped[yvals-30:yvals,win_right-15:win_right+15], axis=0)) < 1:
                rightx[yvals] = win_right
                win_right = win_right
            else:
                rx = win_right-15 + np.argmax(np.sum(warped[yvals-30:yvals,win_right-15:win_right+15], axis=0))
                rightx[yvals] = rx
                win_right = rx
        ...
        #print (win_right)
        window_size_left = 15
        window_size_right = 15
        for yl in range(1,distance+1):
            yvals = distance - yl
            
            if np.sum(np.sum(warped[yvals-10:yvals,win_left-window_size_left:win_left+window_size_left], axis=0)) < 1:
                ...
                lx = win_left-window_size_left + np.argmax(np.sum(warped[yvals-15:yvals,win_left-window_size_left:win_left+window_size_left], axis=0))
                leftx[yvals] = lx
                win_left = lx
            ...
            if np.sum(np.sum(warped[yvals-10:yvals,win_right-window_size_right:win_right+window_size_right], axis=0)) < 1:
                ...
                rx = win_right-window_size_right + np.argmax(np.sum(warped[yvals-15:yvals,win_right-window_size_right:win_right+window_size_right], axis=0))
                rightx[yvals] = rx
                win_right = rx
